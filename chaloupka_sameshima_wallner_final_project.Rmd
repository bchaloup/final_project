...
<!-- If you want to build a model, you might want to check the assumptions of the residuals too. -->
<!-- -From Maggie :) -->
```{r}
library(lm.beta)
library(MASS)
...
confint(model1)
# Create a new column and append the Unstandardized model predicted values

data$pred   <- predict(model1)

# Create a new column and append the Studentized Residuals

data$zresid <- studres(model1)

# Compute the absolute value of studentized residuals

data$abs.zresid <- abs(data$zresid)

# Create a new column and append the Studentized Deleted Residuals

data$stdresid <- rstudent(model1)

# Create a new column and append the Hat Values

data$hat <- hatvalues(model1)
...
seq_fractal_data %>% # change points to numbers for easy sequence position identification
  ...
  # + labs(color = "Sequence Position")
#It may be helpful to have clear labels with spaces in a final manuscript (rather than underscores). I added a legend label above.
